...
func TestInstallerScript_UnixHappyPath(t *testing.T) {
	...
	out, err := cmd.CombinedOutput()
	require.NoError(t, err, string(out))
	assert.Contains(t, string(out), "info: Installed summon at:")
...
func TestInstallerScript_PathOptOutAndFallbackMessages(t *testing.T) {
	...
	assert.Contains(t, string(optOutOutput), "info: PATH update skipped")
...
func TestInstallerScript_QuietModeSuppressesOutput(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell installer test only runs on unix-like systems")
	}
	...
	// In quiet mode, say() and info() output nothing
	outStr := string(out)
	assert.NotContains(t, outStr, "info: Installing summon")
	assert.NotContains(t, outStr, "info: Installed summon at:")

	// Binary should still be installed correctly
	verify := exec.Command(targetPath, "--version")
	verifyOut, err := verify.CombinedOutput()
	require.NoError(t, err, string(verifyOut))
	assert.Contains(t, string(verifyOut), "9.9.9")
...
func TestInstallerScript_NonInteractiveSkipsMenu(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell installer test only runs on unix-like systems")
	}
	...
	// Non-interactive should NOT show welcome banner or menu
	outStr := string(out)
	assert.NotContains(t, outStr, "Welcome to Summon!")
	assert.NotContains(t, outStr, "Proceed with standard installation")
	assert.NotContains(t, outStr, "Cancel installation")
	// But should show the compact install message
	assert.Contains(t, outStr, "info: Installed summon at:")
}

func TestInstallerScript_UpgradeDetectionDisplayed(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell installer test only runs on unix-like systems")
	}
	...
	// Second install (upgrade) — still non-interactive, so no upgrade banner
	// but the binary should still install successfully
	cmd2 := exec.Command("sh", installerScriptPath(t))
	cmd2.Dir = projectDir
	cmd2.Env = baseEnv
	out2, err := cmd2.CombinedOutput()
	require.NoError(t, err, string(out2))
	assert.Contains(t, string(out2), "info: Installed summon at:")
}

func TestInstallerScript_NoColorDisablesANSI(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell installer test only runs on unix-like systems")
	}
	...
	outStr := string(out)
	assert.NotContains(t, outStr, "\033[")
	assert.NotContains(t, outStr, "\x1b[")
}
