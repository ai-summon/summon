...
SUMMON_LATEST_CHECKSUM="${SUMMON_LATEST_CHECKSUM:-}"
SUMMON_RELEASE_BASE_URL="${SUMMON_RELEASE_BASE_URL:-https://github.com/user/summon/releases/download}"
...
resolve_checksum() {
  ...
  if [ "$SUMMON_VERSION" = "latest" ] && [ -n "$SUMMON_LATEST_CHECKSUM" ]; then
    printf '%s' "$SUMMON_LATEST_CHECKSUM"
    return
  fi
  ...
  fail checksum "No checksum source available. Provide SUMMON_CHECKSUM, SUMMON_LATEST_CHECKSUM, or SUMMON_CHECKSUM_URL."
}
...
main() {
  ...
  EXPECTED_CHECKSUM=$(resolve_checksum "$VERSION" "$ARTIFACT_PATH")
